...
tech-product recommendations** powered by either [z.ai](https://z.ai) or [flock.io](https://flock.io).
...
- **Multi-provider support** – lets the user choose `z.ai` or `flock.io`, then uses the matching chat-completion API to surface 3–5
...
python3 scripts/search.py --provider z.ai --save-key "your_zai_api_key_here"
python3 scripts/search.py --provider flock.io --save-key "your_flock_api_key_here"
```
...
|---|---|---|
| `ZAI_API_KEY` | Conditionally | Required when using `z.ai` |
| `FLOCK_API_KEY` | Conditionally | Required when using `flock.io` |
...
| `OPENSEA_PROVIDER` | No | Default provider when `--provider` is not passed |
...
│   └── providers.json           # Allowed models per provider
├── install/

...
description: >
  ...
  product advice. Powered by the z.ai API. Before any workflow, collect the
  ZAI_API_KEY if it is not already set.
---
...
Rules:
- Always check for `ZAI_API_KEY` before running searches.
- If `ZAI_API_KEY` is not set in the environment, ask the user:
  > "Please provide your z.ai API Key to continue."
- Accept the key as plain text or in the form `ZAI_API_KEY=xxx`.
- Save the key to `runtime/.env` via `scripts/search.py --save-key <key>`.
- Never print or log the full key; only confirm: `ZAI_API_KEY received ✓`
...
2. The script drives the questionnaire interactively and calls the z.ai API.
...
- Calls the z.ai API once all context is collected.
...
- Never expose the `ZAI_API_KEY` in outputs or logs.
- Only use model names from `config/providers.json`.
- Do not fabricate product URLs or prices — the z.ai model must provide real sources.
